!function(e) {
    ...
    AFRAME.registerComponent("environment", {
        ...
        getFogColor: function(e, t) {
            ...
            return r.multiplyScalar(.9),
            r.lerp(new THREE.Color(this.data.groundColor), .3),
            "#" + r.getHexString()
        },
        update: function(e) {
            var t;
            this.data.preset ? (t = AFRAME.utils.clone(this.environmentData),
            this.environmentData = {},
            ...
            console.log(this.environmentData)) : (t = e,
            this.environmentData = this.data);
            var r = this.environmentData.skyType
              , i = new THREE.Vector3(this.environmentData.lightPosition.x,this.environmentData.lightPosition.y,this.environmentData.lightPosition.z);
            if (i.normalize(),
            this.sunlight) {
                if (this.sunlight.setAttribute("position", this.environmentData.lightPosition),
                "atmosphere" != r) {
            ...
            if (r !== t.skyType || this.environmentData.skyColor != t.skyColor || this.environmentData.horizonColor != t.horizonColor) {
                ...
                "color" == r ? (n.color = this.environmentData.skyColor,
                n.fog = !1) : "gradient" == r && (n.topColor = this.environmentData.skyColor,
            ...
            this.setStars(2e3 * (1 - Math.max(0, 8 * (i.y + .08))))),
            this.environmentData.fog > 0 ? this.el.sceneEl.setAttribute("fog", {
            ...
            this.hemilight.setAttribute("visible", "none" !== this.environmentData.lighting);
            var a = !this.groundGeometry || this.environmentData.seed != t.seed || this.environmentData.ground != t.ground || this.environmentData.playArea != t.playArea || this.environmentData.flatShading != t.flatShading;
            (a || this.environmentData.groundColor != t.groundColor || this.environmentData.groundColor2 != t.groundColor2 || this.environmentData.groundYScale != t.groundYScale || this.environmentData.groundTexture != t.groundTexture || this.environmentData.gridColor != t.gridColor || this.environmentData.grid != t.grid) && (this.updateGround(a),
            this.hemilight && this.hemilight.setAttribute("light", {
                groundColor: this.environmentData.groundColor
            })),
            this.environmentData.seed == t.seed && this.environmentData.dressingOnPlayArea == t.dressingOnPlayArea && this.environmentData.dressing == t.dressing && this.environmentData.flatShading == t.flatShading && this.environmentData.dressingAmount == t.dressingAmount && this.environmentData.dressingScale == t.dressingScale && this.environmentData.dressingColor == t.dressingColor && this.environmentData.dressingVariance.x == t.dressingVariance.x && this.environmentData.dressingVariance.y == t.dressingVariance.y && this.environmentData.dressingVariance.z == t.dressingVariance.z && this.environmentData.dressingUniformScale == t.dressingUniformScale || this.updateDressing(),
            this.sky.setAttribute("visible", "none" !== r),
            this.el.setAttribute("visible", this.environmentData.active),
            this.environmentData.active || (this.userFog ? this.el.sceneEl.setAttribute("fog", this.userFog) : this.el.sceneEl.removeAttribute("fog")),
        ...
        logPreset: function() {
            ...
            for (var t in this.schema)
                if ("preset" != t) {
                    ...
                    e += "vec3" == r ? "{ x: " + this.environmentData[t].x + ", y: " + this.environmentData[t].y + ", z: " + this.environmentData[t].z + "}" : "string" == r || "color" == r ? '"' + this.environmentData[t] + '"' : this.environmentData[t],
                    e += ", "
                }
            e += "}",
            console.log(e)
            ...
            for (var i in this.schema)
                if (!("preset" == i || r && void 0 === r[i])) {
                    ...
                    if ("vec3" == a) {
                        ...
                        e(s.x) == e(n.x) && e(s.y) == e(n.y) && e(s.z) == e(n.z) || t.push(i + ": " + e(n.x) + " " + e(n.y) + " " + e(n.z))
                    } else
                        o != n && ("number" == this.schema[i].type && (n = e(n)),
                        t.push(i + ": " + n))
                }
        ...
        updateGround: function(e) {
            ...
            if (e) {
                ...
                if (this.ground.setAttribute("visible", i),
                !i)
                    return;
                this.groundGeometry || (this.groundGeometry = new THREE.PlaneGeometry(this.STAGE_SIZE + 2,this.STAGE_SIZE + 2,t - 1,t - 1));
                for (var o = new r, n = this.groundGeometry.vertices, a = this.groundGeometry.vertices.length, s = 10, l = s / t, d = 0, h = 0, g = 0; d < a; d++)
                    if ("flat" != this.environmentData.ground) {
                        ...
                        case "canyon":
                            ...
                            break;
                        case "spikes":
                            c = this.random(d) < .02 ? this.random(d + 1) : 0;
                            break;
                        case "noise":
                            c = this.random(d) < .35 ? this.random(d + 1) : 0
                        }
                        c += .1 * this.random(d + 2);
                        var m = 2 * h / s - 1
                          , u = 2 * g / s - 1
                          , f = this.environmentData.playArea;
                        m = Math.max(0, Math.min(1, (Math.abs(m) - (f - .9)) * (1 / f))),
                        u = Math.max(0, Math.min(1, (Math.abs(u) - (f - .9)) * (1 / f))),
                        c *= m > u ? m : u,
                        c < .01 && (c = 0),
                        n[d].z = c,
                        h += l,
                ...
                this.environmentData.flatShading ? this.groundGeometry.computeFlatVertexNormals() : this.groundGeometry.computeVertexNormals(),
            ...
            var p = 2048
              , v = 20
              , y = this.STAGE_SIZE / v;
            this.groundCanvas && this.groundCanvas.width == p || (this.gridCanvas = document.createElement("canvas"),
            ...
            (new THREE.Material).hasOwnProperty("shading") ? this.groundMaterialProps.shading = this.environmentData.flatShading ? THREE.FlatShading : THREE.SmoothShading : this.groundMaterialProps.flatShading = this.environmentData.flatShading,
        ...
        drawGrid: function(e, t, r) {
            if ("none" != this.environmentData.grid) {
                var i, o, n, a = Math.floor(r / 2), s = t / (r / 2);
                switch (e.fillStyle = this.environmentData.gridColor,
                this.environmentData.grid) {
                case "1x1":
                case "2x2":
                    for ("1x1" == this.environmentData.grid && (a *= 2,
                    s = t / r),
                    i = 0; i < a; i++)
                        n = Math.floor(i * s),
                        e.fillRect(0, n, t, 1),
                ...
                case "crosses":
                    ...
                    for (i = 0; i < a + 1; i++)
                        for (n = Math.floor(i * s),
                        o = 0; o < a + 1; o++) {
                            var d = Math.floor(-l + o * s);
                            e.fillRect(d, n, 2 * l, 1),
                            e.fillRect(n, d, 1, 2 * l)
                        }
                    break;
                case "dots":
                    for (i = 0; i < a + 1; i++)
                        for (o = 0; o < a + 1; o++)
                            e.beginPath(),
                            e.arc(Math.floor(o * s), Math.floor(i * s), 4, 0, 2 * Math.PI),
                            e.fill();
                    break;
                case "xlines":
                    for (i = 0; i < a; i++)
                        e.fillRect(Math.floor(i * s), 0, 1, t);
                    break;
                case "ylines":
                    for (i = 0; i < a; i++)
                        e.fillRect(0, Math.floor(i * s), t, 1)
                }
            }
        },
        drawTexture: function(e, t, i) {
            ...
            if ("none" != this.environmentData.groundTexture)
                ...
                case "checkerboard":
                    ...
                    for (o = 0; o < g + 1; o += 2)
                        for (var m = 0; m < g + 1; m++)
                            e.fillRect(Math.floor((o + m % 2) * c), Math.floor(m * c), Math.floor(c), Math.floor(c));
                    break;
                case "squares":
                    var u = 16
                      , f = t / u;
                    for (a = new THREE.Color(this.environmentData.groundColor),
                    s = new THREE.Color(this.environmentData.groundColor2),
                    o = 0; o < u * u; o++)
                        n = this.random(o + 3) > .5 ? a.clone() : s.clone(),
                        n.addScalar(.1 * this.random(o + 3) - .05),
                ...
                case "walkernoise":
                    ...
                    E.putImageData(d, 0, 0),
                    e.drawImage(x, 0, 0, t, t)
                }
        },
        getAssetGeometry: function(e) {
            function t(e, t) {
                for (var r = new THREE.Vector3, i = 0, o = e.vertices.length; i < o; i++)
                    r.x = (a.random(i) - .5) * t,
                    r.y = (a.random(i + o) - .5) * t,
                    r.z = (a.random(i + 2 * o) - .5) * t,
                    e.vertices[i].add(r)
            }
            if (!e)
                return null;
            for (var r, i, o, n = [], a = this, s = 0; s < e.length; s++)
                if ("lathe" == e[s].type) {
                    ...
                    i.applyMatrix((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(-Math.PI,0,0))),
                    ...
                    i.applyMatrix((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(-Math.PI / 2,0,0))),
                    e[s].noise && t(i, e[s].noise),
                    n.push(i)
                    ...
                    this.environmentData.flatShading || e[s].flatShading ? i.computeFaceNormals() : i.computeVertexNormals(),
                    e[s].mirror && i.merge(i, (new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(0,Math.PI,0))),
                    e[s].noise && t(i, e[s].noise),
                    n.push(i)
                }
            return n
        ...
        updateDressing: function() {
            ...
            if (this.dressing.setAttribute("visible", "none" != this.environmentData.dressing),
            "none" != this.environmentData.dressing) {
                var t, r = new THREE.Geometry;
                switch (this.environmentData.dressing) {
                case "cubes":
                    t = [new THREE.BoxGeometry(1,1,1)],
                    t[0].applyMatrix((new THREE.Matrix4).makeTranslation(0, .5, 0));
                    break;
                case "pyramids":
                    ...
                    break;
                case "cylinders":
                    t = [new THREE.CylinderGeometry(.5,.5,1,8,1,!0)],
                    t[0].applyMatrix((new THREE.Matrix4).makeTranslation(0, .5, 0));
                    break;
                default:
                    if (t = this.getAssetGeometry(this.assets[this.environmentData.dressing]),
                    !t)
                        return
                }
                for (var i = 0, o = 88343; i < this.environmentData.dressingAmount; i++,
                o++) {
                    var n, a = t[Math.floor(this.random(33 + i) * t.length)], s = this.environmentData.dressingScale, l = new THREE.Vector3(this.environmentData.dressingVariance.x,this.environmentData.dressingVariance.y,this.environmentData.dressingVariance.z), d = this.random(o) < this.environmentData.dressingOnPlayArea;
                    n = d ? 15 * this.random(o + 1) : 10 + Math.max(l.x, l.z) + 10 * this.random(o + 1) + this.random(o + 2) * this.STAGE_SIZE / 3;
                    var h = this.random(o + 3) * Math.PI * 2
                      , g = new THREE.Matrix4
                      , c = this.random(o + 4)
                      , m = this.environmentData.dressingUniformScale;
                    g.compose(new THREE.Vector3(Math.cos(h) * n,0,Math.sin(h) * n), (new THREE.Quaternion).setFromAxisAngle(new THREE.Vector3(0,1,0), (this.random(o + 5) - .5) * l.length() * Math.PI * 2), new THREE.Vector3(s + (m ? c : this.random(o + 6)) * l.x,s + (m ? c : this.random(o + 7)) * l.y,s + (m ? c : this.random(o + 8)) * l.z)),
                    r.merge(a, g)
                }
                ...
                e.add(p),
                this.dressing.setObject3D("mesh", e)
            }
        },
        createStars: function() {
            for (var e = 2e3, t = new THREE.BufferGeometry, r = new Float32Array(3 * e), i = this.STAGE_SIZE - 1, o = new THREE.Vector3, n = 0; n < r.length; n += 3)
                o.set(this.random(n + 23) - .5, this.random(n + 24), this.random(n + 25) - .5),
                o.normalize(),
                o.multiplyScalar(i),
                r[n] = o.x,
                r[n + 1] = o.y,
        ...
        setStars: function(e) {
            this.stars || (this.stars = document.createElement("a-entity"),
            this.stars.id = "stars",
            this.createStars(),
    ...
    var r = function(e) {
        ...
        var t;
        for (t = 0; t < 256; t++)
            this.p[t] = Math.floor(256 * e.random(666));
        for (this.perm = [],
        t = 0; t < 512; t++)
