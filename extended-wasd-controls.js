AFRAME.registerComponent("extended-wasd-controls", {
  ...
  tick: function (time, timeDelta) {
    ...
    let lookAmount =
      (timeDelta / 1000) * THREE.Math.degToRad(this.data.lookSpeed);
    ...
    if (this.lookControls) {
      ...
      if (this.data.inputType == "keyboard") {
        ...
        if (this.isKeyPressed(this.data.lookUpKey)) this.rotatePercent.x += 1;
        if (this.isKeyPressed(this.data.lookDownKey)) this.rotatePercent.x -= 1;
        ...
        // center on horizon
        if (
          this.isKeyPressed(this.data.lookUpKey) &&
      ...
      if (this.data.maxLookEnabled) {
        if (this.lookAngle > maxLookAngle) this.lookAngle = maxLookAngle;
        if (this.lookAngle < -maxLookAngle) this.lookAngle = -maxLookAngle;
      }
    }
    ...
    if (this.data.inputType == "keyboard") {
      ...
      if (this.data.flyEnabled) {
        if (this.isKeyPressed(this.data.moveUpKey)) this.movePercent.y += 1;
        if (this.isKeyPressed(this.data.moveDownKey)) this.movePercent.y -= 1;
      }
    } // other, e.g. "joystick"
    else {
